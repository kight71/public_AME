...
class _TorchPolicyExporter(torch.nn.Module):
    ...
    def __init__(self, policy, normalizer=None):
        ...
        # ------------------- Auto-detect terrain encoder (AME CNN/MHA) -------------------
        ...
        # ------------------- Auto-detect terrain MLP encoder -------------------
        self.has_terrain_mlp = hasattr(policy, "terrain_encoder") and not self.has_terrain_encoder
        if self.has_terrain_mlp:
            self.terrain_obs_dim = policy.terrain_obs_dim
            self.terrain_embedding_dim = policy.terrain_embedding_dim
            self.terrain_encoder = copy.deepcopy(policy.terrain_encoder)
        ...
        # ------------------- Forward dispatch (JIT-safe: no unused branches) -------------------
        # Recurrent models already have forward set above; non-recurrent need dispatch here.
        if not self.is_recurrent:
            if self.has_terrain_mlp:
                self.forward = self._forward_terrain_mlp
            elif self.has_terrain_encoder:
                self.forward = self._forward_terrain
            else:
                self.forward = self._forward_plain

    # ---- Forward methods (one per model type, so JIT never sees unused branches) ----

    def _forward_plain(self, x):
        """Forward for models without any terrain encoder (plain MLP)."""
        x = self.normalizer(x)
        return self.actor(x)

    def _forward_terrain_mlp(self, x):
        """Forward for terrain MLP models (ActorCriticTerrainMlp)."""
        if x.dim() == 1:
            x = x.unsqueeze(0)
        proprio_obs = x[:, :-self.terrain_obs_dim]
        terrain_obs = x[:, -self.terrain_obs_dim:]
    ...
    def _forward_terrain(self, x):
        """Forward for CNN/MHA terrain encoder models (AME)."""
        x = self.normalizer(x)
        if x.dim() == 1:
            x = x.unsqueeze(0)
        map_scan_size = self.L * self.W * self.coord_dim
        map_scan = x[:, -map_scan_size:].reshape(-1, self.W, self.L, self.coord_dim)
        proprio_obs = x[:, :-map_scan_size]
        # CNN
...
class _OnnxPolicyExporter(torch.nn.Module):
    ...
    def __init__(self, policy, normalizer=None, verbose=False):
        ...
        if self.has_terrain_encoder:
            ...
            if self.attach_global:
                self.global_encoder.cpu()
                self.query_projector.cpu()
        # Ensure terrain encoder attributes always exist (needed by JIT compilation
        # of _encode_terrain even when has_terrain_encoder=False).
        if not self.has_terrain_encoder:
            self.L = self.W = self.coord_dim = 0
    ...
    def _encode_terrain_mlp(self, obs):
        """Terrain MLP encoding path for ONNX export (aligned with JIT path)."""
        if not self.has_terrain_mlp:
            raise RuntimeError("Terrain MLP encoder is not available in this model.")

        if obs.dim() == 1:
            obs = obs.unsqueeze(0)

        # Split observation: terrain (tail) and proprioception (head)
        proprio_obs = obs[:, :-self.terrain_obs_dim]
        terrain_obs = obs[:, -self.terrain_obs_dim:]

        # Normalize only proprioception (consistent with ActorCriticTerrainMlp._encode_obs)
        proprio_obs = self.normalizer(proprio_obs)

        # MLP terrain encoding
        terrain_embedding = self.terrain_encoder(terrain_obs)

        # Concatenate normalized proprio + terrain embedding
        encoded_obs = torch.cat([proprio_obs, terrain_embedding], dim=-1)
        return encoded_obs

    def forward(self, x):
        if self.has_terrain_mlp:
            x = self._encode_terrain_mlp(x)
        elif self.has_terrain_encoder:
            x = self.normalizer(x)
    ...
    def export(self, path, filename):
        ...
        if self.is_recurrent:
            ...
            if self.has_terrain_mlp:
                # Total input dim = proprio dim + terrain obs dim
                # actor input = proprio_dim + terrain_embedding_dim
                proprio_dim = self.actor[0].in_features - self.terrain_embedding_dim
                input_dim = proprio_dim + self.terrain_obs_dim
